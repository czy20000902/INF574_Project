...
def support_base(voxels_np):
    ...
    for id_y in range(grid_shape[1]):
        if (np.count_nonzero(voxels_np[:, id_y, :] == True) > 0):
            ...
            scaled_base = np.full((grid_shape[0], grid_shape[2]), False)
            ...
            scaled_base[center[0] - 1:center[0] + 1, center[1]:center[1] + 1] = True
            ...
            """
            # pdb.set_trace()
            if ((np.logical_and(scaled_base, base) == scaled_base).all()):
                print("Support base is reasonable!!")

                return scaled_base


def carving(voxel_surface, voxel_inside, base):
...
scaled_base = support_base(voxels)
carved_voxel_inside = carving(voxel_surface, voxel_inside, scaled_base)
